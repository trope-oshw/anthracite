...
| Current draw | **69.9 mA** at 9.0 V (measured) |
| Recommended supply | 200 mA or more |
| Operating voltage | 8.0 – 12.0 V (7.5 – 12.5 V functional range) |
| Input impedance | **16.2 kΩ** (Single) / **28.7 kΩ** (Hum) / **50.4 kΩ** (Wah) at 1 kHz, measured |
| Input impedance, bypassed | **1.0 MΩ** at 100 Hz, measured |
| Output impedance | **1.05 kΩ** at 1 kHz, measured |
| Bypass | Buffered. Switched electronically — no mechanical contacts in the audio path |
| MIDI | 3.5 mm TRS, MIDI Type-A, 31250 baud. IN and OUT/Thru |
| Microcontroller | STM32F030C8T6 (Cortex-M0, 48 MHz, LQFP48) |
| Enclosure | 73.5 (W) × 57.5 (H) × 128 (D) mm including knobs and jacks, 268 g |

Impedance figures are measured on a v1.0 unit by the series-resistor method,
not simulated. They fall to roughly one fifth of the 1 kHz value at 10 kHz;
that roll-off is deliberate and is what the Input switch selects between.
...
|---|---|
| **Volume** | 10 kΩ, log |
| **Fuzz** | 50 kΩ, log. Roughly 39 dB to 64 dB of small-signal gain |
| **Bass** | 20 kΩ, linear. Corner near 1 kHz, −25 dB to +13 dB |
| **Treble** | 20 kΩ, linear. Corner 300–500 Hz, −26 dB to +14 dB |

| Toggle | Positions | What it changes |
|---|---|---|
| **Bias** | Skew / Offset / Centre | Operating point of the second fuzz transistor (3:1, 2:1, 1:1). More asymmetry means more even-order harmonics and less gain. Second harmonic moves from −16.8 dBc (Centre) to −8.2 dBc (Skew) |
| **Response** | Gradual / Medium / Steep | Series resistance at the fuzz input, which sets how abruptly the pedal cleans up as you roll back the guitar volume. THD at 400 Hz / 1 Vpp: 65.3 % / 59.4 % / 49.0 % |
...
always active regardless of the DIP switch settings.

| CC | Function | Values |
|---|---|---|
| **81** | Bypass / engage (channel 12 by default) | ≥ 64 engages, < 64 bypasses |
| **83** | Input | 0 = return to the physical toggle · 1–31 Single · 32–63 Hum · 64–95 Wah · 96–127 all off |
| **84** | Response | 0 = return to the toggle · 1–31 Steep · 32–63 Medium · 64–95 Gradual |
| **85** | Bias | 0 = return to the toggle · 1–31 Centre · 32–63 Offset · 64–95 Skew · 96–127 extra skew |

Sending 0 to any of CC 83/84/85 hands control back to the physical toggle, so a
MIDI override never becomes something you cannot undo from the panel.

**MIDI-Learn** stores a channel and CC number for the bypass function in EEPROM.
Units ship unlearned: with DIP-SW4 set to ON before learning anything, MIDI will
not respond at all. **Active Sensing** (0xFE) is emitted every 250 ms, and
received bytes are passed through to MIDI OUT unmodified, both only in MIDI mode.
...
```
hardware/     KiCad projects for the three boards, with the symbol and
...
**All Trope branding has been removed from the published panel artwork**, so a
panel made from these files carries no Trope marks by default. Please keep it
that way. The circuit boards are published exactly as manufactured.
